...
func TestMockPlannerBuildsThreeCandidatesFromDeterministicPreviews(t *testing.T) {
	...
	if !reflect.DeepEqual(plan.SelectedIssueIDs, []string{"i-1", "i-3"}) {
		t.Fatalf("unexpected selected issues: %#v", plan.SelectedIssueIDs)
	}
	if !reflect.DeepEqual(plan.AutoRepairIssueIDs, []string{"i-1", "i-3"}) {
	...
	if !reflect.DeepEqual(stringsFromAny(plan.ProposedPayload["issue_ids"]), []string{"i-1", "i-3"}) {
